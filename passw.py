...
def readFile(file):
    ...
    for i in range(lineCounter(file) - 1):
        with open(file, "r") as passwFile:
            ...
            print(
                colored(string_i + " -> ", "yellow")
                + colored(d["name"], "green")
                + colored("\nEmail: ", "blue")
                + colored(d["email"], "cyan")
            )
...
def userId():
    ...
    if i == "1":
        ...
        for x in range(lineCounterBytes("usrID.json") - 1):
            ...
            if (
                authantication(x)["username"] == username
                and authantication(x)["password"] == password
            ):
                ...
                createUsrID_json = open(
                    "passwUsers/" + authantication(x)["username"] + ".json", "a"
                )
                return 2

        if (
            authantication(x)["username"] != username
            or authantication(x)["password"] != password
        ...
        if usrMatch == 0:
            ...
            if q == "":
                ...
                sha256_encoded_string = hashlib.sha256(
                    encoded_password + salt
                ).hexdigest()

                usrDic = {
                    "username": username,
                    "password": sha256_encoded_string,
